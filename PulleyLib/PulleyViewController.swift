...
public class PulleyViewController: UIViewController, UIScrollViewDelegate, PulleyPassthroughScrollViewDelegate {
    ...
    public var primaryContentViewController: UIViewController! {
        willSet {
    ...
    public var drawerBackgroundVisualEffectView: UIVisualEffectView? = UIVisualEffectView(effect: UIBlurEffect(style: UIBlurEffectStyle.ExtraLight)) {
        ...
        didSet {
            ...
            if let drawerBackgroundVisualEffectView = drawerBackgroundVisualEffectView where self.isViewLoaded()
            {
    ...
    var supportedDrawerPositions: [PulleyPosition] = PulleyPosition.all {
        didSet {
    ...
    override public func loadView() {
        ...
        dimmingViewTapRecognizer = UITapGestureRecognizer(target: self, action: #selector(PulleyViewController.dimmingViewTapRecognizerAction(_:)))
    ...
    override public func viewDidLayoutSubviews() {
        ...
        let borderPath = UIBezierPath(roundedRect: drawerContentContainer.bounds, byRoundingCorners: [UIRectCorner.TopLeft, UIRectCorner.TopRight], cornerRadii: CGSize(width: drawerCornerRadius, height: drawerCornerRadius)).CGPath
    ...
    public func setDrawerPosition(position: PulleyPosition, animated: Bool = true)
    {
        ...
        if animated
        {
            UIView.animateWithDuration(0.3, delay: 0.0, usingSpringWithDamping: 0.75, initialSpringVelocity: 0.0, options: UIViewAnimationOptions.CurveEaseInOut, animations: {
                self.drawerScrollView.setContentOffset(CGPoint(x: 0, y: stopToMoveTo - lowestStop), animated: false)
                
                if let drawer = self as? PulleyViewController
                {
    ...
    public func setPrimaryContentViewController(controller: UIViewController, animated: Bool = true)
    {
        if animated
        {
            UIView.transitionWithView(primaryContentContainer, duration: 0.5, options: UIViewAnimationOptions.TransitionCrossDissolve, animations: {
                self.primaryContentViewController = controller
            }, completion: { (cmopleted) in
    ...
    public func setDrawerContentViewController(controller: UIViewController, animated: Bool = true)
    {
        if animated
        {
            ...
            UIView.transitionWithView(drawerContentContainer, duration: 0.5, options: UIViewAnimationOptions.TransitionCrossDissolve, animations: {
    ...
    public func scrollViewWillEndDragging(scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>)
    {
        if scrollView == drawerScrollView
        {
            lastDragTargetContentOffset = CGPoint(x: CGFloat(targetContentOffset.memory.x), y: CGFloat(targetContentOffset.memory.y))
            
            // Halt intertia
            targetContentOffset.memory.x = scrollView.contentOffset.x
